...
def no_adj_dup(ll):
    if ll is None:
        return None
    front = rear = LN(ll.value)     # init first node and rear node
    ll = ll.next                    # get next node
    while ll != None:               # traverse remain linkedlist
        value = ll.value            # get current value
        ll = ll.next                # get next node
        if value != rear.value:     # if current value != last node's value
            rear.next = LN(value)   # append the new node to the rear.next
            rear = rear.next
    return front
...
def no_adj_dup_r(ll):
    ...
    front = rear = LN(ll.value)     # init first node and rear node
    ll = ll.next                    # get next node
    if ll is None:                  # if ll has only one node, return
        return front
    if ll.value == rear.value:      # if current node's value == rear.value, jump to next node
        ll = ll.next
    res = no_adj_dup_r(ll)
    if res and res.value == rear.value:     # if result's first node's value=rear.value
        res = no_adj_dup_r(ll.next)
    rear.next = res    # recursive get pair_sum
    rear = rear.next
    return front        
...
def bases(aclass):
    class_bases = set(aclass.__bases__) # get aclass's base class
    temp = set()
    for base_class in class_bases:      # recursive get base class
        temp = temp.union(bases(base_class))
    class_bases = class_bases.union(temp)
    return class_bases
