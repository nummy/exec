...
def isNumber(item):
    ...
    if type(item) == int or type(item) == float:
        return True
    return False  # HINT You already did this on in a previous lab
...
def isListOfIntegers(theList):
    ...
    # Now we can assume that theList really is a list
    # But is it a list of all integers?
    # If we find even a single item that isn't integer, we can
    # immediately return false.
    ...
    # If we get here and didn't return yet, then we know everything
    # in the list is a simple integer!
    # (i.e. there isn't anything in the list that is NOT simple integer)
